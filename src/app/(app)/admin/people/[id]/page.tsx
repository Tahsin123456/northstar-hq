...
export default function EmployeeProfilePage({
  ...
  // side. See the matching note on the People list.
      ...
      >
        <DialogContent className="max-w-md">
          ...
          <DialogBody className="flex flex-col gap-3">
            ...
            <p className="text-[12px] leading-relaxed text-muted-foreground">
              If you change your mind, reactivate them from their row on People.
            </p>
...
function PayConfiguration({ payroll }: { payroll: EmployeePayrollDTO }) {
  return (
    <dl className="grid gap-x-6 gap-y-4 sm:grid-cols-3">
      ...
      {/*
        LABELLED FOR WHAT IT NOW IS. This column priced every hit until the rate
        moved to the niche, and it still explains every payslip finalized before
        that. Leaving it labelled "Per hit" would read as the rate this person
        is paid today, which is the one thing it is not — an admin would set it,
        watch nothing change, and lose trust in the screen.
      */}
      <Detail label="Per hit (historical)">
        <span className="tnum text-[15px] font-medium text-foreground">
          {formatMoneyTrimmed(payroll.hitPaymentMinor, payroll.currency)}
        </span>
        <p className="mt-1 max-w-[32ch] text-[11px] leading-relaxed text-subtle-foreground">
          What a hit paid before the rate moved to the niche. It explains earlier
          payslips and no longer decides new pay —{" "}
          <Link href="/admin/niches" className="text-accent hover:underline">
            each niche sets its own hit payment
          </Link>
          .
        </p>
      </Detail>
        ...
        {period.byNiche.length === 0 ? (
          ...
          period.byNiche.map((bucket) => (
            <CalculationRow
              ...
              // The NICHE's rate, off the line, not the person's. A record can
              // hold two niches at two prices, so one figure from the profile
              // would explain half of these rows with the wrong number. A
              // frozen line that cannot name its price says the count alone.
              detail={
                bucket.hitPaymentMinor === null
                  ? `${formatNumber(bucket.hitCount)} ${pluralize(bucket.hitCount, "hit")}`
                  : `${formatNumber(bucket.hitCount)} ${pluralize(bucket.hitCount, "hit")} × ${formatMoneyTrimmed(bucket.hitPaymentMinor, currency)}`
              }
...
function CalculationRow({
  ...
  hint?: string;
  /**
   * `null` when the amount genuinely is not known — a frozen line whose record
   * was paid at several per-niche rates, which `PayrollRecord` has one column
   * for. An em dash rather than a zero: "we cannot break this month down by
   * niche" and "this niche earned nothing" are different statements, and only
   * one of them is true.
   */
  amount: number | null;
  currency: string;
  ...
  return (
    <div className="flex items-baseline justify-between gap-4 py-1.5">
      ...
      <span className="tnum shrink-0 text-[13px] text-foreground">
        {amount === null ? EM_DASH : formatMoneyTrimmed(amount, currency)}
      </span>
    </div>
  );
}
  ...
  return (
    <form onSubmit={submit}>
      <DialogHeader>
        ...
        <DialogDescription>
          The monthly salary, which feeds the payroll run directly. What a hit
          pays is set per niche now, so the per-hit figure here only explains
          payslips finalized before that changed.
        </DialogDescription>
      </DialogHeader>
      ...
      <DialogBody className="flex max-h-[60vh] flex-col gap-4 overflow-y-auto">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="pay-hit">Payment per hit (historical)</Label>
            <Input
              id="pay-hit"
              inputMode="decimal"
            ...
            {hitPaymentError ? (
              ...
              /*
                STILL EDITABLE, AND SAYS WHAT IT DOES. This is the rate that
                priced hits before the rate moved to the niche, and it explains
                every payslip finalized under it — which is why it is here at
                all. An admin who typed a number in and saw no future run change
                would rightly conclude the screen was broken, so the field says
                so before they do it, rather than after.
              */
              <FieldHint>
                No longer used for new pay — each niche sets what a hit in it is
                worth. Kept because earlier payslips were calculated from it.
              </FieldHint>
            )}
          </div>
        </div>
...
/*
 * `minorToInputText` used to live here, beside an identical copy in the finance
 * ledger's entry form. It is one export in `money.ts` now, next to the
 * `parseMoneyToMinor` it is the inverse of — two private copies of a round trip
 * is two chances for one of them to stop being the inverse of anything.
 */
