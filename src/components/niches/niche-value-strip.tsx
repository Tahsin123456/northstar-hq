...
import { isStillMeasuring } from "@/lib/analytics/niche-earnings";
import {
  VIEWS_GAINED_UNAVAILABLE,
  measuredChannelsCaption,
} from "@/lib/analytics/views-gained-labels";
 ...
 * is Northstar's.
 *
 * THE VIEWS ARE GAINS, NOT UPLOADS' LIFETIMES AND NOT LIFETIME TOTALS. The
 * strip prices the views the tracked channels GAINED during the selected
 * period — each channel's counter at the close minus the same counter at the
 * start, split between Shorts and long-form by the mix of its uploads —
 * fetched once per page through `useNicheViewsGained` and handed down; this
 * component adds no data fetching of its own. The card's other stats (Shorts
 * count, hit rate) stay on the upload basis deliberately; they answer "how
 * did recent output do?", the money answers "what did the period pay?".
 ...
 * YouTube pays, not about how many people watched, and understating reach to
 * explain money would be a second wrong number solving the first.
 *
 * ---------------------------------------------------------------------------
 ...
 * add an exchange rate, wait for the history, wait for the import.
 *
...
export function NicheValueStrip({
   ...
   * endpoint omitted. `null` renders as "measuring", never as zero.
   */
   ...
   * how old the newest reading is when that is worth saying. Page-level: the
   * span is a fact about the history, the same for every card.
   */
  measuredNote: string | null;
  /** When the view history began, for the definition's own sentence. */
  historyBeganMs: number | null;
}) {
  const rpm = niche.rpm;
      ...
      <div className="flex items-baseline justify-between gap-2">
        <span className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-subtle-foreground">
          ...
          {/* The qualifier the label cannot carry: WHICH views were priced —
              the views gained during the period, split between the formats
              by an estimate, measured only as far back as the recorded
              history reaches. Selected on the niche's own format: the Shorts
              sentence about engaged views would state the OPPOSITE of a Long
              Form niche's arithmetic. */}
          <InfoTip>{trackedNicheValueDefinition(format, historyBeganMs)}</InfoTip>
        </span>
...
/** The gains-dependent body: money, "nothing gained", or "measuring". */
function MeasuredValue({
  ...
  /*
   * THE SAME RULE THE OVERVIEW PANEL HOLDS, through the same predicate.
   *
   * A niche none of whose channels holds two readings has an UNKNOWN gain —
   * the first reading was taken and the second comes with the next refresh —
   * and "no views gained" would be a claim the measurement never made. Words
   * instead, saying when. The rate still renders below: it is a fact about
   * the niche, not about the period.
   */
  const measured =
    gained === null
      ? null
      : { measuredChannels: gained.measuredChannels, totalChannels: gained.totalChannels };
  if (isStillMeasuring(measured)) {
    return (
      <div className="flex flex-col gap-1">
        <span
          className="text-[12px] text-subtle-foreground"
          aria-label={`${TRACKED_NICHE_VALUE_LABEL}: ${NICHE_MEASURING}.`}
        >
          {NICHE_MEASURING}
        </span>
        <p className="text-[11px] leading-relaxed text-muted-foreground">
  ...
  // Which part of the niche the figure covers, when that is not all of it.
  // Beside the number rather than in a tooltip: the missing channels are
  // unknown, not zero, and a bare figure would read as the whole niche.
  const partial = measured === null ? null : measuredChannelsCaption(measured);
  ...
  return (
    <>
      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
        ...
        {partial !== null ? (
          <span className="text-[11px] text-subtle-foreground">{partial}</span>
        ) : null}
      </div>
...
/**
 * A niche with no rate at all — neither derived nor entered.
 *
 ...
 * on every one of its four states; the em dash is this app's symbol for an
 * empty period, and reusing it here would say the niche was measured and came
 * up empty. It is also not hidden: a card with no strip at all would make an
 * unpriced niche indistinguishable from one nobody has channels in.
 */
