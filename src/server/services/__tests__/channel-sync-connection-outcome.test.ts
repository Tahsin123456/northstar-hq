...
vi.mock("@/server/db", () => ({
  prisma: {
    channel: {
      ...
      // The upsert answers with the counters YouTube returned, the way the
      // real one does — the channel reading below is written from them.
      upsert: vi.fn(async () => ({
        ...channelRow,
        viewCount: BigInt(freshChannel.viewCount),
        subscriberCount: BigInt(freshChannel.subscriberCount),
        videoCount: BigInt(freshChannel.videoCount),
      })),
