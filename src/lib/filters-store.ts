 ...
 *
 * A FACTORY NOW, NOT A SINGLETON — because there are two products. The Shorts
 * pages and the Long Form pages are two independent analysis surfaces, and a
 * niche selected on one must not narrow the other, so each side gets its own
 * store instance under its own storage key. `createFiltersStore` closes over
 * ALL FOUR pieces of what used to be module state (`serverSnapshot`,
 * `clientSnapshot`, `listeners`, `lastSeenSearch`); the module still exports
 * the Shorts singleton's functions under their historical names, so every
 * existing import site keeps meaning exactly what it always meant.
 *
 * THE URL IS SHARED BETWEEN THE INSTANCES, DELIBERATELY. Both stores read and
 * write the same parameter names (`period`, `threshold`, `niche`, …), because
 * the two products live on disjoint routes: a Next `<Link>` into /longform
 * carries no query string across, so at any moment the URL belongs to exactly
 * one page and therefore one store. Namespacing the params would buy
 * protection against only a hand-crafted cross-product link — whose failure is
 * already soft (a niche id from the other format resolves to no niche) — at
 * the price of every shared link and every `?niche=` navigation growing a
 * prefix.
 */
...
/** The Long Form pages' own store. v1 — this key has no history to migrate. */
const LONGFORM_STORAGE_KEY = "northstar-hq:filters:longform:v1";
...
/**
 * One product's filter store — everything the module's named exports used to
 * be, as an object, so a second product can hold its own without sharing a
 * niche selection with the first.
 */
export interface FiltersStore {
  /** Seeds the server-render defaults. Idempotent; called during render. */
  readonly seedDefaults: (defaults: { period: PeriodSelection }) => void;
  readonly subscribeToFilters: (listener: () => void) => () => void;
  readonly getFiltersSnapshot: () => FiltersSnapshot;
  readonly getFiltersServerSnapshot: () => FiltersSnapshot;
  /** Forces the next read to re-derive from the URL (Next client navigation). */
  readonly invalidateFiltersFromUrl: () => void;
  ...
  readonly setThreshold: (value: number) => void;
  /** Drops the override and returns to the niche (or account) default. */
  readonly clearThresholdOverride: () => void;
  readonly setNicheFilter: (niche: NicheFilter) => void;
  readonly setContentTypeFilter: (contentType: ContentTypeFilter) => void;
  readonly setOwnershipFilter: (ownership: OwnershipFilter) => void;
  readonly setOwnFirst: (ownFirst: boolean) => void;
  readonly resetFilters: () => void;
}

export function createFiltersStore(storageKey: string): FiltersStore {
   ...
   *
   * This is what makes the URL the real source of truth. Without it the
   * snapshot was read exactly once per page load, so navigating from the
   * Niches page to `/?niche=RDR` returned the *previous* niche — and
   * `persist()` then wrote that stale value straight back into the URL,
   * silently undoing the navigation.
   */
  let lastSeenSearch: string | null = null;

  /**
   * Seeds the defaults that the server rendered with, so the server snapshot
   * and the first client render agree. Called once, before hydration.
   */
  function seedDefaults(defaults: { period: PeriodSelection }): void {
    ...
    // Only the analysis defaults come from the server; the view filters
    // (niche, ownership) are always session state and reset to "show
    // everything".
  ...
  function readFromEnvironment(): FiltersSnapshot {
    ...
    try {
      ...
      if (stored) {
        ...
        // Absent from anything stored before content types existed, which is
        // why this reads defensively rather than bumping the storage key: an
        // old snapshot is still entirely valid, it simply has no opinion here.
      ...
      // Corrupt JSON or storage blocked in a private window. Not worth failing
      // a page load over — fall through to the defaults.
    }
    ...
    try {
      ...
      // written when the user chose one, so a shared link reproduces exactly
      ...
      if (urlNiche) {
        ...
        // carried-over override, for the same reason setNicheFilter does: the
        // new niche's own threshold should apply. An explicit `threshold=` in
        // the same link is a deliberate choice by the sender and still wins.
  ...
  function persist(snapshot: FiltersSnapshot): void {
    ...
    try {
      ...
      // Only present when overridden, so the default view keeps a clean URL
      // and niche navigation is not pinned to a stale number.
      ...
      // replaceState, not the Next router: a segmented control should not push
      // a history entry or re-render the route tree on every click.
  ...
  function subscribeToFilters(listener: () => void): () => void {
    listeners.add(listener);

    // Back/forward changes the URL without re-rendering through React's
    // router, so the store has to hear about it directly for history
    // navigation to restore the right niche and period.
    const onPopState = () => {
   ...
   */
  function invalidateFiltersFromUrl(): void {
    lastSeenSearch = null;
    for (const l of listeners) l();
  }

  function getFiltersSnapshot(): FiltersSnapshot {
  ...
  function getFiltersServerSnapshot(): FiltersSnapshot {
  ...
  function setPeriodPreset(preset: PeriodPresetId): void {
    const current = getFiltersSnapshot();
    commit({ ...current, period: { ...current.period, preset } });
  }

  function setCustomRange(startMs: number, endMs: number): void {
    const current = getFiltersSnapshot();
    commit({
      ...current,
  ...
  /** Drops the override and returns to the niche (or account) default. */
  function clearThresholdOverride(): void {
    commit({ ...getFiltersSnapshot(), thresholdOverride: null });
  }

  function setNicheFilter(niche: NicheFilter): void {
    const current = getFiltersSnapshot();
    if (current.niche === niche) return;
    // A threshold override is scoped to the niche it was made in. Carrying it
    // across a niche switch would mean selecting a niche no longer applies
    // that niche's own definition of a hit — so moving to a different niche
    // drops the override and falls back to that niche's default.
   ...
   *
   * Unlike `setNicheFilter`, this does not touch the threshold override. A
   * niche carries its own definition of a hit, so switching niche has to drop
   * an override made under the previous one; a content type has no threshold
   * of its own and never will — what counts as a hit is a property of the
   * audience, not of the format — so there is nothing here for an override to
   * contradict.
   */
  function setContentTypeFilter(contentType: ContentTypeFilter): void {
    const current = getFiltersSnapshot();
    if (current.contentType === contentType) return;
    commit({ ...current, contentType });
  }

  function setOwnershipFilter(ownership: OwnershipFilter): void {
    commit({ ...getFiltersSnapshot(), ownership });
  }

  function setOwnFirst(ownFirst: boolean): void {
...
/**
 * The Shorts product's store — the singleton this module has always been.
 *
 * Its functions are re-exported below under their historical names, so the
 * refactor to a factory is invisible to every existing import site: same
 * storage key, same URL behaviour, same function identities for the lifetime
 * of the module.
 */
export const defaultFiltersStore: FiltersStore = createFiltersStore(STORAGE_KEY);

/**
 * The Long Form product's store. Same machinery, its own storage key, so a
 * niche selected on /longform never narrows a Shorts page and vice versa.
 * Created here rather than in a Long Form module so both instances' lifetimes
 * are the same story in the same file; an unused instance holds no listeners
 * and costs nothing.
 */
export const longformFiltersStore: FiltersStore = createFiltersStore(LONGFORM_STORAGE_KEY);
